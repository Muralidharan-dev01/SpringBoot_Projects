...
//        Category category= savedList.stream().filter(c->c.getCategoryId()
//                .equals(categoryId)).findFirst().orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND,"Resource Not Found"));
//          //categories.stream().filter("same").findFirst().orElse(null);
//        if(category==null)
...
//        List <Category> savedList= categoryRepository.findAll();
//        Optional<Category> optionalCategory=savedList.stream().filter(c->c.getCategoryId().
